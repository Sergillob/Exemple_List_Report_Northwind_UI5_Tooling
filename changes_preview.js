	...
	function loadChanges() {
		...
		function prepareChanges() {
			...
			if (usePreviewConnector()) {
				...
				oChanges = {
					changes: oResult,
					componentClassName: "sl.elements.exemple_list_report_northwind_ui5_tooling"
				};
			}
		...
		return new Promise(function (resolve, reject) {
			$.ajax({
				...
				function isMTAChange(sPath) {
					...
					return sPath.startsWith("Exemple_List_Report_Northwind_UI5_Tooling") && sPath.indexOf("webapp/changes/") > 0 &&
						isValidChangeFileExt(sPath);
				}
